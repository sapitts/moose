...
template <>
InputParameters validParams<CrystalPlasticityKalidindiUpdate>()
{
...
void
CrystalPlasticityKalidindiUpdate::calculateConstitutiveEquivalentSlipIncrement(RankTwoTensor & equivalent_slip_increment,
                                                                         bool & error_tolerance)
{
  ...
  for (unsigned int i = 0; i < _number_slip_systems; ++i)
  {
    _slip_increment[_qp][i] = _ao * std::pow(std::abs(_tau[_qp][i] / _slip_system_resistance[_qp][i]), 1.0 / _xm);
    if (_tau[_qp][i] < 0.0)
...
void
CrystalPlasticityKalidindiUpdate::updateConstitutiveSlipSystemResistanceAndVariables(bool & error_tolerance)
{
