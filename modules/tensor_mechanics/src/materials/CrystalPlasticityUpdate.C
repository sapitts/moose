...
template <>
InputParameters
validParams<CrystalPlasticityUpdate>()
{
  InputParameters params = validParams<Material>();
  params.addClassDescription("Crystal Plasticity base class: handles the Newton iteration over the stress residual and calculates the Jacobian based on constitutive laws provided by inheriting classes");
  ...
  params.addRequiredParam<FileName>("slip_sys_file_name", "Name of the file containing the slip systems");
  params.addParam<Real>("number_cross_slip_directions", 0, "Quanity of unique slip directions, used to determine cross slip familes");
  params.addParam<Real>("number_cross_slip_planes", 0, "Quanity of slip planes belonging to a single cross slip direction; used to determine cross slip families");
  ...
  params.addParam<Real>("stol", 1e-2, "Constitutive internal state variable relative change tolerance");
  params.addParam<Real>("slip_increment_tolerance", 2e-2, "Maximum allowable slip in an increment");
  params.addParam<Real>("resistance_tol",2.0e-2,"Constitutive slip system resistance residual tolerance");
...
CrystalPlasticityUpdate::postSolveQp(RankTwoTensor & cauchy_stress, RankFourTensor & jacobian_mult)
{
  ...
  _total_lagrangian_strain[_qp] = _deformation_gradient[_qp].transpose() * _deformation_gradient[_qp] - RankTwoTensor::Identity();
...
CrystalPlasticityUpdate::solveStateVariables()
{
  ...
  while (iter_flag && iteration < _maxiterg)
  {
    ...
    {
#ifdef DEBUG
    mooseWarning("CrystalPlasticityUpdate: state variables did not converge at element ", _current_elem->id(), " and qp ", _qp, "\n");
#endif
    }
    iteration++;
  }
  ...
  {
#ifdef DEBUG
    mooseWarning("CrystalPlasticityUpdate: Hardness Integration error. Reached the maximum number of iterations to solve for the state variables at element ", _current_elem->id(), " and qp ", _qp, "\n");
#endif
  ...
  for (unsigned int i = 1; i < _number_slip_systems; ++i)
  {
    for (unsigned int j = 0; j < family_counter; ++j)
    {
      ...
      for (unsigned int k = 0; k < LIBMESH_DIM; ++k)
      {
        ...
        dot_product += std::abs(_slip_direction(check_family_index * LIBMESH_DIM + k) - _slip_direction(i * LIBMESH_DIM + k));
      }
      ...
      if (MooseUtils::absoluteFuzzyEqual(dot_product, 0.0))
      {
        ...
        if (_cross_slip_familes[j].size() > _number_cross_slip_planes)
          mooseError("Exceeded the number of cross slip planes allowed in a single cross slip family");

        break; // exit the loop over the exisiting cross slip families and move to the next slip direction
      }
