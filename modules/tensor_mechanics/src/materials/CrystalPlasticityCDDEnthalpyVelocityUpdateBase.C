...
#include "CrystalPlasticityCDDEnthalpyVelocityUpdateBase.h"
...
CrystalPlasticityCDDEnthalpyVelocityUpdateBase::CrystalPlasticityCDDEnthalpyVelocityUpdateBase(const InputParameters & parameters) :
    ...
    _previous_it_resistance(declareProperty<std::vector<Real> >("previous_interation_athermal_slip_system_resistance")),
...
// void
// CrystalPlasticityCDDEnthalpyVelocityUpdateBase::initialSetup()
// {
//   // fetch coupled gradients of the plastic velocity gradient
//   for (unsigned int i = 0; i < LIBMESH_DIM * LIBMESH_DIM; ++i)
//     _gradient_Lp[i] = &coupledGradient("plastic_velocity_gradient_components", i);
...
{
  _athermal_slip_system_resistance[_qp] = _athermal_slip_system_resistance_old[_qp];
...
void
CrystalPlasticityCDDEnthalpyVelocityUpdateBase::calculateConstitutiveEquivalentSlipIncrement(RankTwoTensor & equivalent_slip_increment,
...
void
CrystalPlasticityCDDEnthalpyVelocityUpdateBase::calculateGlideSlipIncrement(bool & error_tolerance)
{
  ...
  for (unsigned int i = 0; i < _number_slip_systems; ++i)
  {
    ...
    if (std::abs(_glide_slip_increment[_qp][i]) > _slip_incr_tol)
    {
      ...
      std::cout << "  the slip system resistance is " << _athermal_slip_system_resistance[_qp][i] << std::endl;
#endif
      error_tolerance = true;
      return;
    }
...
void
CrystalPlasticityCDDEnthalpyVelocityUpdateBase::calculateConstitutiveSlipDerivative(std::vector<Real> & dslip_dtau, unsigned int /*slip_model_number*/)
{
...
void
CrystalPlasticityCDDEnthalpyVelocityUpdateBase::calculateGlideSlipDerivative(std::vector<Real> & dslip_glide_dtau)
{
    const Real activation_term = _activation_energy / (_boltzmann_constant * _temperature);
    ...
    for (unsigned int i = 0; i < _number_slip_systems; ++i)
    {
      ...
      if (driving_force > 0.0) // Need a positive driving force to move the dislocations
      {
        ...
        dslip_glide_dtau[i] = _mobile_dislocations[_qp][i] * _burgers_vector * velocity_derivative * _substep_dt;
      }
    }
}

void
CrystalPlasticityCDDEnthalpyVelocityUpdateBase::updateConstitutiveSlipSystemResistanceAndVariables(bool & error_tolerance)
{
...
bool
CrystalPlasticityCDDEnthalpyVelocityUpdateBase::areConstitutiveStateVariablesConverged()
{
  ...
  for (unsigned int i = 0; i < _number_slip_systems; ++i)
  {
    resistance_diff = std::abs(_previous_it_resistance[_qp][i] - _athermal_slip_system_resistance[_qp][i]);
...
void
CrystalPlasticityCDDEnthalpyVelocityUpdateBase::calculateDislocationDensities(bool & error_tolerance)
{
  ...
  for (unsigned int i = 0; i < _number_slip_systems; ++i)
  {
    ...
    const Real term4_force = std::sqrt(std::abs(_tau[_qp][i] / _athermal_slip_system_resistance[_qp][i]));
...
Real
CrystalPlasticityCDDEnthalpyVelocityUpdateBase::calculateMeanFreeGlidePath()
{
...
void
CrystalPlasticityCDDEnthalpyVelocityUpdateBase::calculateDislocationCrossSlip()
{
...
void
CrystalPlasticityCDDEnthalpyVelocityUpdateBase::calculateGeometricallyNecessaryDislocations()
{
  ...
  for (unsigned int i = 0; i < LIBMESH_DIM; ++i)
  {
    ...
    {
      if (j == 0)
        increment_nyes_tensor(i,0) = (*_gradient_Lp[i * LIBMESH_DIM + 2])[_qp](1) - (*_gradient_Lp[i * LIBMESH_DIM + 1])[_qp](2);
      else if (j == 1)
        increment_nyes_tensor(i,1) = (*_gradient_Lp[i * LIBMESH_DIM + 0])[_qp](2) - (*_gradient_Lp[i * LIBMESH_DIM + 2])[_qp](0);
      else if (j == 2)
        increment_nyes_tensor(i,j) = (*_gradient_Lp[i * LIBMESH_DIM + 1])[_qp](0) - (*_gradient_Lp[i * LIBMESH_DIM + 0])[_qp](1);
    }
  }
...
void
CrystalPlasticityCDDEnthalpyVelocityUpdateBase::calculateSlipSystemResistance(bool & error_tolerance)
{
...
void
CrystalPlasticityCDDEnthalpyVelocityUpdateBase::calculateDislocationForestHardening(
