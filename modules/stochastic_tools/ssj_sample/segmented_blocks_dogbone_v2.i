...
[Mesh]
  ...
  [move_top_tab]
    ...
    input = 'top_tab'
    transform = TRANSLATE
    vector_value = '${fparse -1.0 * pitch_length - 0.5 * top_center_gauge_portion} ${fparse pitch_length + top_gauge_height + 0.5 * gauge_transition_height} 0'
  []
  [rename_top_tab]
    type = RenameBlockGenerator
    input = 'move_top_tab'
  ...
  [stitch_top_tab]
    type = StitchedMeshGenerator
    inputs = 'rename_top_shoulder_top_side rename_top_tab'
