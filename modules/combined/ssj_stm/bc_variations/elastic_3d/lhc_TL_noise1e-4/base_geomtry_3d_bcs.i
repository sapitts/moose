...
[Mesh]
  ...
  [extruder]
    ...
    num_layers = '${fparse thickness / 0.1}'
    bottom_sideset = back
    top_sideset = front
  []
[]
...
[Materials]
  [elasticity_tensor]  ## should replace these values with zirconium
...
[Postprocessors]
  ...
  [p1_vonmises_stress]
    type = PointValue
    variable = vonmises_stress
    point = '0 ${fparse 0.5 * gauge_height + 1.4} ${fparse thickness/2.0}'
  []
  [p2_vonmises_stress]
    type = PointValue
    variable = vonmises_stress
    point = '0 ${fparse 0.5 * gauge_height} ${fparse thickness/2.0}'
  []
  [p3_vonmises_stress]
    type = PointValue
    variable = vonmises_stress
    point = '0 ${fparse 0.25 * gauge_height} ${fparse thickness/2.0}'
  []
  [p4_vonmises_stress]
    type = PointValue
    variable = vonmises_stress
    point = '${fparse -0.5*gauge_width} 0 ${fparse thickness/2.0}'
  []
  [p5_vonmises_stress]
    type = PointValue
    variable = vonmises_stress
    point = '0 0 ${fparse thickness/2.0}'
  []
  [p6_vonmises_stress]
    type = PointValue
    variable = vonmises_stress
